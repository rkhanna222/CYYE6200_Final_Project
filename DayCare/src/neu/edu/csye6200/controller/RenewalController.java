    ...
    public void renewTheRecord(){
        int selectedRow = renewalJPanel.getTblRenewalRecords().getSelectedRow();
         if(selectedRow>=0){
                ...
                List<Student> student = StudentDataMangementFactory.getFactoryInstance().getObject().getDataList(Integer.parseInt(studentId));
                if(!student.isEmpty()){
            ...
            JOptionPane.showMessageDialog(null, "Please select a row from table first to renew!","Warning",JOptionPane.WARNING_MESSAGE);
            return;
        }
